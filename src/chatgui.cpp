...
ChatBotFrame::ChatBotFrame(const wxString &title) : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(width, height))
{
    ...
    client.client_start();
    //async(std::launch::async, &Client::client_recieve, client);
...
void ChatBotFrame::OnEnter(wxCommandEvent &WXUNUSED(event))
{
    ...
    _panelDialog->PrintChatbotResponse("HI");
    client.client_send(std::string(userText.mb_str()));
    //std::cout<<"hhhhhhhhh\n";
    ...
    : wxPanel(parent, -1, wxPoint(-1, -1), wxSize(-1, -1), wxBORDER_NONE)
{
    ...
    _chatBotImg = new wxStaticBitmap(this, wxID_ANY, wxBitmap(imgBasePath + "user.png", wxBITMAP_TYPE_PNG), wxPoint(-1, -1), wxSize(-1, -1));
